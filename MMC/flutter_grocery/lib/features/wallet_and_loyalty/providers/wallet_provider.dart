...
class WalletAndLoyaltyProvider with ChangeNotifier {
  ...
  Future<Map<String, dynamic>?> createRazorpayOrder(double amount) async {
    try {
      ...
      ApiResponseModel apiResponse = await walletRepo!.createRazorpayOrder(amount);
  ...
  Future<bool> addFundViaRazorpay({
    ...
    try {
      ...
      ApiResponseModel apiResponse = await walletRepo!.addFundViaRazorpay(
        paymentId: paymentId,
        orderId: orderId,
        signature: signature,
        amount: amount,
      ...
      if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
        // Refresh the wallet transaction list
        getLoyaltyTransactionList('1', true, true);
        return true;
      } else {
        ApiCheckerHelper.checkApi(apiResponse);
        return false;
      ...
      _isLoading = false;
      notifyListeners();
      print('Error adding fund via Razorpay: $e');
      return false;
    }
  }
  ...
  Future<bool> verifyRazorpayPayment({
    ...
    try {
      ApiResponseModel apiResponse = await walletRepo!.verifyRazorpayPayment(
        paymentId: paymentId,
        orderId: orderId,
        signature: signature,
      );
