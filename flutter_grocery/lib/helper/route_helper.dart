...
static String getPaymentRoute({String? id = '', String? url, PlaceOrderModel? placeOrderBody, String? paymentMethod}) {
  ...
  return '$payment?id=$id&uri=$uri&place_order=$data&method=$method';
}
...
static final Handler _paymentHandler = Handler(handlerFunc: (BuildContext? context, Map<String, dynamic> params) {
  String paymentMethod = params['method'] != null ? params['method'][0] : '';
  
  // For Razorpay payments, use the SDK integration
  if (paymentMethod == 'razor_pay') {
    try {
      final orderData = utf8.decode(base64Decode(params['place_order'][0]));
      final PlaceOrderModel orderBody = PlaceOrderModel.fromJson(jsonDecode(orderData));
      
      // Get customer email from context - you might need to adjust this based on your app structure
      ...
      return _routeHandler(
        child: RazorpayPaymentScreen(
          ...
          onPaymentSuccess: (String paymentId, String razorpayOrderId) {  },
          
        ),
      );
    } catch (e) {
      // Fallback to error screen or home screen if there's an error parsing data
      return _routeHandler(child: const HomeScreen());
    }
  } else {
    // For other payment methods, you can implement web-based payments if needed
    // For now, redirect to home screen
    return _routeHandler(child: const HomeScreen());
  }
});
// Update the payment route method to include payment method:
